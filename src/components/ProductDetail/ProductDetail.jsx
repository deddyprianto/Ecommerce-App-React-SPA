...
const ProductDetail = () => {
  ...
  const { id } = useParams();
  const { dataResponse, isLoading, isError } = useProducts("products");
  const dataSpesified = dataResponse.filter((item) => item.id === Number(id));
  ...
  return (
    <>
      {isLoading ? (
        ...
        <p className="text-red-500">Terjadi kesalahan {isError.message}</p>
      ) : (
        <>
          {dataSpesified.map((item, i) => (
            <div
              key={i}
              className="2xl:container mt-10 2xl:mx-auto lg:py-16 lg:px-20 md:py-12 md:px-6 py-9 px-4 "
            >
              <div className="flex justify-center items-center lg:flex-row flex-col gap-8">
                <div className="w-full sm:w-96 md:w-8/12 lg:w-6/12 items-center">
                  <p
                    onClick={() => navigate("/shop", { replace: true })}
                    className="text-blue-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-800 font-normal text-base leading-4 cursor-pointer mb-2"
                  >
                    Back
                  </p>
                  <p className=" focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-800 font-normal text-base leading-4 text-gray-600">
                    Home / DetailProduct / {item.title}
                  </p>
                  ...
                  <div className="lg:mt-11 mt-10">
                    ...
                    <div
                      className={`flex flex-row justify-between items-start mt-4  ${
                        rotate && "h-20 scale-up-ver-top  overflow-y-auto"
                      }`}
                    >
                      {rotate ? (
                        <div className="w-full flex-col justify-evenly h-full">
                ...
                <div className=" w-full sm:w-96 md:w-8/12  lg:w-6/12 flex lg:flex-row flex-col lg:gap-8 sm:gap-6 gap-4">
                  ...
                  <div className=" w-full lg:w-4/12 grid lg:grid-cols-1 sm:grid-cols-4 grid-cols-2 gap-6">
                    <div className="flex justify-center items-center py-4">
                      <img src={item.imgSrc} alt="Wooden chair - preview 1" />
                    </div>
                    <div className="flex justify-center items-center py-4">
                      <img src={item.imgSrc} alt="Wooden chair - preview 2" />
                    </div>
                    <div className="flex justify-center items-center py-4">
                      <img src={item.imgSrc} alt="Wooden chair- preview 3" />
                    </div>
                  </div>
                </div>
              </div>
              <div className="flex  justify-center items-center w-full">
                <div className="w-full sm:w-96 md:w-8/12 lg:w-full grid lg:grid-cols-2 md:grid-cols-2 grid-cols-1 lg:gap-28 sm:gap-x-6 sm:gap-y-12 gap-y-12 sm:mt-14 mt-10 ">
                  {item.moreDescription.map((item, i) => (
                    <div key={i}>
                      <svg
                        width="50"
                        height="50"
