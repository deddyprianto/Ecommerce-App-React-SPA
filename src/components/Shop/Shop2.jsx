...
const MyCustomComponent = ({ item }) => (
  <div className=" relative ">
    ...
    <div className=" relative group">
      ...
      <img className=" w-full" src={item.src} alt="A girl Posing Img" />
...
const Shop2 = () => {
  const { dataResponse, isLoading, isError } = useProducts("SummerCollection");
  return (
      ...
      <div className=" py-6 lg:px-20 md:px-6 px-4">
        ...
        <div className=" grid lg:grid-cols-4 sm:grid-cols-2 grid-cols-1 lg:gap-y-12 lg:gap-x-8 sm:gap-y-10 sm:gap-x-6 gap-y-6 lg:mt-12 mt-10 ">
          {isLoading ? (
            ...
            <p className="text-red-500 text-center">Error {isError.message}</p>
          ) : (
            dataResponse?.map((item, i) => (
              <MyCustomComponent item={item} key={i} />
            ))
          )}
        </div>
